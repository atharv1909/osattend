...
import { supabase, type Session, type Attendance } from "@/lib/supabase"
...
export default function TeacherPage() {
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      const { data: attendees } = await supabase
        .from('attendance')
        .select('*')
        .eq('session_code', currentSession.code)
        .order('marked_at', { ascending: false })

      setAttendanceData({
        session: { ...session, is_expired: isExpired },
        attendees: attendees || [],
  ...
  const exportCSV = () => {
    ...
    try {
      ...
      const csvRows = attendanceData.attendees
        .map(a => {
          const name = a.student_name.replace(/"/g, '""')
          const roll = a.roll_number.replace(/"/g, '""')
      ...
      a.href = url
      a.download = `${attendanceData.session.subject.replace(/\s+/g, '_')}_attendance.csv`
